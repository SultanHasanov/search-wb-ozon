  ...
  for (let index = 0; index < products.length; index++) {
    ...
    try {
      if (product.product?.image) {
        try {
          // Скачиваем оригинал
          const response = await axios.get(product.product.image, {
            responseType: "arraybuffer",
          });

          // Уменьшаем через sharp (например, до 300x300)
          const resizedBuffer = await sharp(response.data)
            .resize(300, 300, { fit: "cover" }) // можно fit: 'inside', если хочешь сохранить пропорции
            .toBuffer();

          // Отправляем фото из буфера
